...
<body>
	...
	<div id="albumes">
		...
		if (!$resultado)
		{
			die("<script>jQuery(function(){sweetAlert('Opss...', 'Error al obtener tus albumes', 'error');});</script>");
		}else{
			if(mysqli_num_rows($resultado)>0){
    			while($row=mysqli_fetch_array($resultado)){
    				echo '<div class="box2">';
    				echo '<a href="verFotos.php"><img class="resize"src="'.$row['Portada'].'"></a>';
				...
				die("<script>jQuery(function(){sweetAlert('Opss...', 'Todavía no has creado ningún album', 'warning');});</script>");
			}
		}
		?>
		</div>
		...
		xmlhttp.onreadystatechange = function()
			{
				if (xmlhttp.readyState==4 && xmlhttp.status==200)
				{
					var respuesta=xmlhttp.responseText;
						...
						jQuery(function(){sweetAlert('Opss...', 'Todavía no has creado ningún album!', 'warning');});
					}else{
